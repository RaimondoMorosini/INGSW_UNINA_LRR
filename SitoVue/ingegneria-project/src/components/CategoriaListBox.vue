            ...
            <transition leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100"
                ...
                <ListboxOptions
                    ...
                    <ListboxOption v-for="option in props.options" :key="option.label" v-slot="{ active, selected }"
                        :value="option.value" as="props.as">
                        <li :class="[
                            active ? 'bg-primario-100/30 text-primario-900' : 'text-gray-900',
                            'relative cursor-default select-none py-2 pl-10 pr-4',
                            option.label === 'indietro' ? 'border-b border-gray-200 ' : '',
                        ]">
                            <span :class="[
                            ...
                            <span v-if="option.label === 'indietro'"
                                class="flex absolute inset-y-0 left-0 items-center pl-3 text-amber-600">
                                <ArrowLeftIcon aria-hidden="true" class="w-5 h-5" />
                            </span>
                            <span v-if="option.hasChild"
                                class="flex absolute inset-y-0 right-0 items-center pr-3 text-amber-600">
