<template>
    <div class="card p-fluid flex flex-wrap gap-3">
        <div class="sezione-superiore w-full">
            <BarraRicercaPerOrdine @ordineSelezionato="setCampiOrdinamento($event)"
                @prezzoMinSelezionato="setPrezzoMin($event)" @prezzoMaxSelezionato="setPrezzoMax($event)" />
        </div>

        <div class="sezione-centrale">
            <!--SEZIONE FILTRI CARETTERISTICHE DEL PRODTTO-->
            <CaratteristicheProdotto v-if="props.propRicaricaComponenteCaratteristiche"
                :propCategoria="props.propCategoriaCercata" @caratteristicheSelezionate="setCaratteristiche($event)" />
                ...
                <DataView :value="products">
                    <template #list="slotProps">
                        <div class="grid-nogutter grid">
                            <div @click="apriUnNuovoTab(item.idAsta)" v-for="(item, index) in slotProps.items" :key="index" class="col-12 mt-4 contenitore-articolo">
                                <div class="flex-column sm:align-items-center flex gap-3 bg-zinc-100 p-4 sm:flex-row"
                                    :class="{ 'border-top-1 surface-border': index !== 0 }">
                                    <div class="flex-column sm:align-items-center flex gap-3 bg-zinc-100 p-4 sm:flex-row"
                                        :class="{ 'border-top-1 surface-border': index !== 0 }">
                                        <div class="md:w-10rem relative">
                                            <img class="border-round mx-auto block w-full xl:block" :src="`${item.immagini[0]}`" :alt="item.nome" />
                                            <Tag :value="item.tipoAsta" :severity="danger" class="absolute" style="left: 4px; top: 4px"></Tag>
                                        </div>
                                        <div
                                            ...
                                            <div
                                                ...
                                                <div>
                                                    <span class="text-secondary text-sm font-medium">{{ item.categoria
                                                        }}</span>
                                                    <div class="text-900 mt-2 text-lg font-medium titolo-articolo">
                                                        {{ item.nome }}
                                                    </div>
                                                    <div>
                                                        Venditore: {{ item.emailUtenteCreatore }}
                                                    </div>
                                                    <span class="text-900 text-xl font-semibold">BASE ASTA: EURO{{item.baseAsta }}</span>
                                                </div>
                                            </div>
                                            <div class="flex-column md:align-items-end flex gap-5">
                                                <div
                                                    ...
                                                    <Button @click="apriUnNuovoTab(item.idAsta)"
                                                        icon="pi pi-shopping-cart" label="Partecipa all'asta" :disabled="item.inventoryStatus === 'OUTOFSTOCK'" 
