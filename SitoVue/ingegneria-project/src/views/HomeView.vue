...
<template>
  <SwipeCarosello />
  <div class="card md:space-x-2 space-x-0 space-y-3 md:space-y-0 md:flex-row flex-col sm:static flex md:items-center justify-content-center justify-between py-5 lg:px-6col md:px-[100px] px-4">

    <InputText v-model="value" placeholder="Search" class="rounded border-transparent hover:ring-indigo-300 !focus:ring-indigo-300 bg-indigo-200 text-black md:w-14rem min-w-2col">
      <template #imputtexticon>
        <i class="pi pi-search"></i>
      </template>  
    </InputText>
    
          <TreeSelect v-model="selectedCategory" :options="cities" optionLabel="name" placeholder="Seleziona Categoria" class="rounded !hover:ring-indigo-300 !focus:ring-indigo-300 bg-indigo-200 text-black md:w-14rem min-w-2col">
