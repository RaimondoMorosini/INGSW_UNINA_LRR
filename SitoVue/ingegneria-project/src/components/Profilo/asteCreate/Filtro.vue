<template>
  <div class="filtri p-4 bg-gray-50 rounded-lg shadow-md grid grid-cols-1 lg:grid-cols-4 gap-6">
    ...
    <!-- Ordine -->
    <div>
      <label for="ordine" class="block text-sm font-medium text-gray-700">
        Ordina per
      </label>
      <Dropdown
        id="ordine"
...
watch(() => astaStore.filtroRicerca, (newValue) => {
  filtroRicerca.value = newValue;
});
watch(() => astaStore.filtroTipoAsta, (newValue) => {
  filtroTipoAsta.value = newValue;
});
watch(() => astaStore.filtroScadenza, (newValue) => {
  filtroScadenza.value = newValue;
});
watch(() => astaStore.ordine, (newValue) => {
